...
## 2024-05-24 - Exception Handling as Control Flow in Parsers
**Learning:** Using `try...except ValueError` for type-casting as control flow (like checking if a string is a float/int) is extremely slow in Python when the exception is expected frequently (e.g., parsing common column identifiers).
**Action:** Implement a fast path (e.g., checking if the string starts with an alphabetical character) to bypass the exception logic for common cases. When bypassing `float()`, always account for `'inf'`, `'nan'`, and `'infinity'`, as `float()` successfully parses these specific alphabetical strings.
