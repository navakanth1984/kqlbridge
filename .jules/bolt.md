...
## 2024-07-19 - ValueError overhead in parsing control flow
**Learning:** Using `try...except ValueError` for type-casting as control flow (like checking if a string is a float/int) is extremely slow in Python when the exception is expected frequently (e.g., parsing common identifiers in `_token_to_expr`). Implementing a fast path (e.g., checking if the string starts with an alphabetical character) avoids this significant overhead, reducing parsing time by roughly 80-90% for typical identifiers.
**Action:** Always add a fast path character check to bypass `int()` or `float()` parsing for pure strings/identifiers, ensuring not to bypass valid alphabetical floats like `'inf'`, `'nan'`, and `'infinity'`.
