...
## 2026-05-17 - Fast Path for String Token Parsing Overheads
**Learning:** Using `try...except ValueError` for type-casting as control flow (like checking if a string is a float/int) is extremely slow in Python when the exception is expected frequently (e.g., parsing common string identifiers). Additionally, applying `string.lower()` before verifying if it is necessary creates a full O(N) memory copy, wasting time on standard column names.
**Action:** Implement a fast path (e.g., checking if the string starts with an alphabetical character) to bypass slow cast attempts. Only call `.lower()` after confirming the fast path condition, and handle special alphabetical floats (`'inf'`, `'nan'`) correctly.
