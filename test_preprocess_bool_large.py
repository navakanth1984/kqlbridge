...
test_str = "DeviceEvents | where ActionType == 'Logon' and (InitiatingProcessFileName in ('cmd.exe', 'powershell.exe') or has('Invoke-')) | extend is_evil = has('evil')" * 10
import timeit
print("Original:", timeit.timeit("from __main__ import _preprocess_bool_funcs, test_str; _preprocess_bool_funcs(test_str)", number=100))
print("Optimized:", timeit.timeit("from __main__ import _preprocess_bool_funcs_optimized, test_str; _preprocess_bool_funcs_optimized(test_str)", number=100))
