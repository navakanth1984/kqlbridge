...
class TranslationMemory:
    """
    KQLBridge Translation Memory.
    
    A self-contained thread-safe telemetry and self-correcting dynamic translator that:
    ...
    def load_default_memory(self) -> None:
        ...
        if os.path.exists(default_path):
            try:
                ...
                import logging
                logging.warning("[kqlbridge] Translation Memory failed to load packaged default memory: %s", e)

    def load_memory(self) -> None:
        """Load telemetry and overrides from local JSON store. Thread-safe."""
        with self._lock:
            self.memory = {
                "overrides": {},
                "rules": [],
            ...
            if os.path.exists(self.memory_path):
                try:
                    ...
                    import logging
                    logging.warning("[kqlbridge] Translation Memory failed to load memory database from %s: %s", self.memory_path, e)

    def save_memory(self) -> None:
        """Commit memory database to the local JSON file. Thread-safe."""
        with self._lock:
            try:
                ...
                import logging
                logging.warning("[kqlbridge] Translation Memory failed to save memory database to %s: %s", self.memory_path, e)

    def clear(self) -> None:
        """Reset Translation Memory database and telemetries. Thread-safe."""
        with self._lock:
            self.memory = {
                "overrides": {},
                "rules": [],
    ...
    def learn(self, kql: str, error: Optional[str] = None, fix_sql: Optional[str] = None) -> None:
        """
        Record translation telemetries and learn override solutions. Thread-safe.
    ...
    def register_rule(self, pattern: str, mapping: str) -> None:
        """
        Register a custom Bridge Meta-Language (BML) translation rule. Thread-safe.
    ...
    def recall(self, kql: str) -> Optional[str]:
        """
        Search memory for an exact override or a matching Bridge Meta-Language rule. Thread-safe.
        ...
        for rule in rules_copy:
            ...
            # Note: We execute matching outside the lock, which is thread-safe as rule contents are static dicts.
