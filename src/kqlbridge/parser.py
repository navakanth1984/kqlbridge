...
# ⚡ Bolt Optimization: Pre-compiled regex tokenizer
# Replaces slow O(N) character-by-character string iteration loops with C-optimized regex token matching.
# Impact: Reduces parsing overhead for deeply nested or long argument lists in case()/bool functions.
# The pattern correctly groups strings (accounting for backslash escapes), ignores parens/commas inside strings,
# and uses `.` as a fallback to avoid silently skipping malformed tokens.
_CASE_TOKEN_RE = _re.compile(
    r"""'(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*"|\(|\)|,|[^'"(),]+|.""",
    _re.DOTALL
)
...
def _preprocess_case(kql: str) -> str:
    ...
    while True:
        ...
        for token_match in _CASE_TOKEN_RE.finditer(kql, open_paren_idx + 1):
