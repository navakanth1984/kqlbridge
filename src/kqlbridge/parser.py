...
def _split_case_args(arg_str: str) -> list[str]:
    # ⚡ Bolt Optimization: Replacing manual char-by-char loop with O(n) Regex finditer
    # C-optimized matching dramatically improves tokenization for heavily nested strings
    args = []
    current = []
    paren_depth = 0
    
    for match in _SPLIT_CASE_ARGS_RE.finditer(arg_str):
        token = match.group(0)
        if token == '(':
            paren_depth += 1
