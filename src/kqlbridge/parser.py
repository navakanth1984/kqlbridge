...
def _split_case_args(arg_str: str) -> list[str]:
    ...
    for match in _SPLIT_TOKEN_RE.finditer(arg_str):
        token = match.group(0)
        if token == '(':
            paren_depth += 1
        elif token == ')':
    ...
    if last_idx < len(arg_str):
        args.append(arg_str[last_idx:].strip())
    elif last_idx == len(arg_str) and len(arg_str) > 0 and arg_str[-1] == ',':
         args.append("")
    return args
...
def _preprocess_case(kql: str) -> str:
    ...
    while True:
        ...
        for token_match in _TOKEN_RE.finditer(kql, open_paren_idx + 1):
