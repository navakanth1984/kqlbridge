# Graph Report - kqlbridge  (2026-05-18)

## Corpus Check
- 24 files · ~16,870 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 459 nodes · 754 edges · 19 communities (18 shown, 1 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 111 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `41e16430`
...
1. `SparkSQLGenerator` - 58 edges
2. `sql()` - 57 edges
3. `ExplainResult` - 23 edges
4. `_build_expr()` - 20 edges
5. `LintIssue` - 19 edges
6. `_build_bool_expr()` - 15 edges
7. `TSQLGenerator` - 15 edges
8. `LintResult` - 14 edges
9. `v0.1 Scope — 14 Operators` - 14 edges
10. `TestWhere` - 13 edges
...
Cohesion: 0.08
Nodes (33): JoinOp, KQLQuery, LetBinding, | join kind=inner (RightTable | ...) on key, | join kind=inner (RightTable | ...) on key, | union T2, T3  or  | union (T2 | where ...), T3, let errors = AppLogs | where Level == 'Error';, The root AST node. Represents a complete KQL query.      let_bindings → will bec (+25 more)
...
Cohesion: 0.1
Nodes (14): examples/fabric_migration/01_simple_filter.py ==================================, The routing agent pattern from DE-Context Kit.     Routes each query to the most, route_query(), _check(), _lint(), cli.py — KQLBridge command-line interface ======================================, _translate(), detect_operators() (+6 more)
...
- **Why does `translate()` connect `Community 8` to `Community 1`, `Community 10`, `Community 4`, `Community 6`?**
  _High betweenness centrality (0.248) - this node is a cross-community bridge._
- **Why does `sql()` connect `Community 1` to `Community 8`?**
  _High betweenness centrality (0.208) - this node is a cross-community bridge._
- **Why does `SparkSQLGenerator` connect `Community 0` to `Community 2`, `Community 3`, `Community 4`, `Community 6`, `Community 9`?**
  _High betweenness centrality (0.177) - this node is a cross-community bridge._
- **Are the 41 inferred relationships involving `SparkSQLGenerator` (e.g. with `ExplainResult` and `KQLQuery`) actually correct?**
  _`SparkSQLGenerator` has 41 INFERRED edges - model-reasoned connections that need verification._
